---
title: "`dplyr` Practice Homework - Solutions"
output:
  html_document:
    toc: true
    toc_float: true
...
#3
# We have fewer missing values in the brand variable. Most of the observations that are missing brand are also missing all other observations. This means it might be quite sensible to drop the rows where brand is missing. If would be less sensible to drop the observations that are missing carbohydrates as we still have a lot of useful information in those rows. However, in same cases (like a specific study of carbohydrates in beer) we might want to only include rows with carbohydrate data. 
```
