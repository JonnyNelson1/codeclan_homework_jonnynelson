...
**Task - 1 min**. What do you predict the exponential with $a=5$ and $b=1$ will look like?<br><br> [**Hint** try thinking about what $y$ values you will get for different values of $x$ e.g. $1, 2, 3, 0, -1$ etc]
<details>
<summary>**Solution**</summary>
```{r}
data <- data %>%
  mutate(y7 = exponential(x, a = 5, b = 1))

exp_plot +
  geom_line(data = data, aes(x = x, y = y7), col = "green")

```
Not very exponential looking! But $5 \times 1^x=5$ for any $x$!
</details>
</blockquote>
<br>
...
In maths, we say that a function is the **inverse** of another function if it 'undoes' it. For example:

* the 'square root' function is the inverse of the 'square' function, and vice-versa (although there is a subtlety here to do with signs)
* the 'minus 2' function is the inverse of the 'plus 2' function, and vice-versa
* the 'divide by 9' function is the inverse of the 'multiply by 9' function, and vice-versa

If two functions are each the inverse of the other, then when we plot them, it will look as if they are 'reflected' in a line sloping upwards at $45^{\circ}$ to the $x$-axis (the line $y = x$). 

Let's see this for the two functions $y = 2^x$ and $y = \log_{2}(x)$, which are inverses of each other. 

(PS this code will raise a warning, as we can't take the $\log()$ of zero or a negative number, but `ggplot()` will filter out `NA`s and `Inf` as it plots, which is handy)
...
The functions look like they are the mirror image of each other in the diagonal line, confirming that they are inverses. This would be true for $y = 3^x$ and $y = \log_{3}(x)$, $y = 4^x$ and $y = \log_{4}(x)$, $y = \exp(x)$ and $y = \ln(x)$ and so on for all bases.

## 'Undoing' an exponential

So, one of the main uses of logs and exponentials you'll encounter is to 'undo' the other and 'get at' variables that are inside the log or exponential (or, in more 'mathsey' terms, make a variable the **subject** of an equation).

For example, 'get' $x$ in the following expression:

$$y = 9.44 = \exp(x)$$

Take the log to the appropriate base of both sides. In this case `exp()` means we're using base $e$, so $\ln()$ is the correct log.
