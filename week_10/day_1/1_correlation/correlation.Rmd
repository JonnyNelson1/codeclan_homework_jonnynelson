...
Take the example below, which contains data on father-son height pairs. In your head, try to label the below table with $x_1$ and $y_1$ and the plot with $(x_1,y_1)$. 

![](images/father_son_table.png){#id .class width=45%} ![](images/father_son_plot.png){#id .class width=45%}
...
An easier way of thinking about whether two variables are positively or negatively correlated is to look at the general shape of the data points: if they both move in the same direction (when one increases, the other increases and when one decreases, the other decreases) they are positively correlated, and if they move in the opposite directions (when one increases, the other decreases and vice versa) they are negatively correlated. No correlation means there is no relationship in how the data points vary.
...
You can calculate the Pearson correlation coefficient in `R` via the `cor()` function (Pearson is the default, but other types of correlation can be calculated as well).
...
So we would say that these variables are very strongly negatively correlated. Let's see if the order of variables matters:
...
Take a look at the four plots below:

![](images/anscombes_quartet.png){#id .class width=80%}
...
summarise(anscombe, cor(x2, y2))
summarise(anscombe, cor(x3, y3))
summarise(anscombe, cor(x4, y4))
```
...
These four datasets are actually referred to as _Anscombe's Quartet_ and should hopefully help you remember the importance of visualising data before analysing it, as well as the effect of outliers on descriptive statistics. I highly recommend reading more about [Anscombe's Quartet](https://en.wikipedia.org/wiki/Anscombe%27s_quartet), because you'll see that the correlation coefficient isn't the only statistical property these four datasets have in common!
...
Imagine a study on the positive effects of exercise with two variables measured:

* `amount_of_exercise` in a given week.
* `happiness` in a given week.

If we discover a positive correlation between these variables, we would be tempted to suggest that it is an increase in the amount of exercise that **causes** an increase in happiness, however it is also possible that the causal direction is the opposite i.e. happier people exercise more. 

A well-known example of the directionality problem is whether exposure to violence in video games or on TV causes aggressive behaviour or whether someone with aggressive behaviour is just more likely to consume violent media. 

As both of these cases apply to something as complex as human behaviour, there is also a high chance that there are confounding variables that need to be accounted for.
...
1. **Randomly** split our experimental sample into two groups: 'control' and 'treatment', and try to make sure sample elements with potentially confounding factors are spread as evenly as possible over both groups and/or restricted to a small range
    * e.g. if age is a potential confounder, use stratified sampling by age to form the control and treatment groups, or limit the experiment to say 20-30 year old people 
...
<center>
![](images/Randomised.png){#id .class width=40% height=40%}
</center>
</div>
<br>
...
* Choose a few combinations of variables to plot as $y$ versus $x$ and calculate the correlation coefficients for the same combinations. Do you find any strong correlations?
...
* [Cool interactive visualization of correlations](https://rpsychologist.com/correlation/)
* [A good overview of different correlation methods](https://easystats.github.io/correlation/articles/types.html)
